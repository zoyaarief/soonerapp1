...
function setEnterBtnState({ label = "Enter queue", disabled = false, showPeople = true } = {}) {
...
function loadTimerState(){
  try { return JSON.parse(localStorage.getItem(TIMER_STORE_KEY)) || { startedAt:null, paused:false, pauseLeftSec:null }; }
  catch { return { startedAt:null, paused:false, pauseLeftSec:null }; }
}
function saveTimerState(){ localStorage.setItem(TIMER_STORE_KEY, JSON.stringify(timerState)); }
function clearTimerState(){ timerState = { startedAt:null, paused:false, pauseLeftSec:null }; localStorage.removeItem(TIMER_STORE_KEY); }
function hideTimer(){ clearInterval(countdownT); countdownT = null; qs("#timerRow")?.classList.add("hidden"); }
function secondsLeft(){
  if (!timerState.startedAt) return null;
  if (timerState.paused && typeof timerState.pauseLeftSec === "number") return timerState.pauseLeftSec;
...
function startCountdownIfNeeded(){
  ...
  countdownT = setInterval(()=>{
    if (timerState.paused){ renderCountdown(); return; }
    const left = secondsLeft();
    renderCountdown();
    if (left === 0){
      clearInterval(countdownT);
...
async function loadVenue(){
  ...
  const cover = qs("#cover"); if (cover) cover.src = v.heroImage || "./images/restaurant.jpg";
  ...
  const g = qs("#gallery");
  if (g && Array.isArray(v.gallery)) g.innerHTML = v.gallery.map(src=>`<img src="${src}" alt="">`).join("");
}

async function loadAnnouncements(){
  const ann = await fetchJSON(`/api/announcements/venue/${encodeURIComponent(currentId)}`);
  if (Array.isArray(ann) && ann.length){
    const a = ann[0];
    const banner = qs("#announcement"), icon = qs("#announcementIcon"), t = qs("#announcementText");
    if (banner && icon && t){
...
async function loadReviews(){
  const r = await fetchJSON(`/api/reviews/venue/${encodeURIComponent(currentId)}`);
  const avgEl = qs("#avgRating"), totalEl = qs("#totalReviews"), list = qs("#reviewsList");
  if (r && (r.total || 0) > 0){
    if (avgEl) avgEl.textContent = r.avgRating ? Number(r.avgRating).toFixed(1) : "—";
    if (totalEl) totalEl.textContent = r.total || 0;
    if (list) list.innerHTML = r.items.map(x => `
      <div class="review">
        <div class="row">
...
async function joinQueue(people){
  ...
  try{
    ...
    if (!r.ok){ toast("Failed to join queue"); return; }
    toast("✅ Joined queue");
    toggleQueueButtons(true);
    await refreshMetrics();
  }catch(e){ toast("Failed to join queue"); }
}

async function cancelQueue(){
  try{
    let r = await fetch(`/api/queue/${encodeURIComponent(currentId)}/cancel`, { method:"POST", credentials:"include" });
    if (r.status === 404){
    ...
    if (!r.ok){ toast("Failed to cancel"); return; }
    toast("❌ You left the queue");
    clearTimerState();
    hideTimer();
    toggleQueueButtons(false);
    await refreshMetrics();
  }catch(e){ toast("Failed to cancel"); }
}

async function arrived(){
  try{
    let r = await fetch(`/api/queue/${encodeURIComponent(currentId)}/arrived`, { method:"POST", credentials:"include" });
    if (r.status === 404){
    ...
    timerState.paused = true;
    timerState.pauseLeftSec = secondsLeft() ?? TIMER_DURATION_SEC;
    saveTimerState(); renderCountdown();
...
async function refreshMetrics(){
  const m = await fetchJSON(`/api/queue/metrics/${encodeURIComponent(currentId)}`);
  ...
  text(qs("#status"), (m.openStatus === "open" && m.walkinsEnabled && m.queueActive) ? "Open" : "Closed");

  const seatsLeft = (m.capacity && (m.capacity.spotsLeft ?? null));
  const seatsEl = qs("#seatsLeft"); if (seatsEl) text(seatsEl, seatsLeft ?? "—");

  const peopleInp = qs("#people");
  const people = Number(peopleInp?.value || 2);
  const capacityOK = (seatsLeft == null) || (typeof seatsLeft === "number" && people <= seatsLeft);
  const venueAccepting = (m.openStatus === "open" && m.walkinsEnabled && m.queueActive) && capacityOK;

  const inQueue = Number.isFinite(m.position);
  toggleQueueButtons(inQueue);

  if (inQueue) {
    setEnterBtnState({ label: "Enter queue", disabled: false, showPeople: false });
  } else {
    if (venueAccepting) setEnterBtnState({ label: "Enter queue", disabled: false, showPeople: true });
    else setEnterBtnState({ label: "Queue Unavailable", disabled: true, showPeople: false });
  }

  // Timer rules
  if (!inQueue){ hideTimer(); clearTimerState(); }
  else {
    if (m.position <= 5) startCountdownIfNeeded();
    else { if (timerState.startedAt) startCountdownIfNeeded(); else hideTimer(); }
  }
}
...
function bindUI(){
  ...
  qs("#checkedIn")?.addEventListener("click", () => toast("Waiting for owner to mark you served"));
  ...
  form?.addEventListener("submit", async (e)=>{
    ...
    const textEl = qs("#revText"), ratingEl = qs("#revRating");
    if (!textEl || !ratingEl) return;
    const textV = (textEl.value || "").trim();
    const ratingV = Number(ratingEl.value || 5);
    if (!textV){ toast("Write something first"); return; }
    try{
      const r = await fetch("/api/reviews/add", {
        method:"POST", credentials:"include",
        headers:{ "Content-Type":"application/json" },
        body: JSON.stringify({ venueId: currentId, rating: ratingV, comment: textV })
      });
      if (r.ok){ textEl.value=""; toast("Review posted"); await loadReviews(); return; }
    }catch{}
    // fallback local
    ...
    textEl.value=""; toast("Review posted (local)"); await loadReviews();
  });
}
...
(async function init(){
  ...
  setEnterBtnState({ label: "Queue Unavailable", disabled: true, showPeople: false });
